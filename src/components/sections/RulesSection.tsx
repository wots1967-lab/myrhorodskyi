...
const rules = [
  { icon: Banknote, text: 'Сесія підтверджується повною передоплатою не пізніше ніж за 24 години до початку' },
  { icon: Clock, text: 'Якщо сесія пропускається або скасовується пізніше — вона вважається проведеною' },
  { icon: Heart, text: 'Екстрена ситуація? Напиши — домовимося і перенесемо без питань' },
  { icon: Home, text: "Для сесії потрібне тихе комфортне місце та стабільний відео/аудіозв'язок" },
];
      ...
      <div className="container-custom glass-card-dark rounded-3xl p-8 md:p-12 lg:p-16 shadow-lg">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {rules.map((rule, index) => (
            <div key={index} className="reveal revealed bg-primary-foreground/10 rounded-2xl p-6 text-center"
