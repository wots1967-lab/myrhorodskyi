...
const TherapyCard = ({ step, index, isMobile }: { step: typeof steps[0]; index: number; isMobile: boolean }) => {
  return (
    <motion.div
      ...
      className="group relative rounded-2xl border bg-card/30 p-6 md:p-7 cursor-default transition-all duration-300 overflow-hidden border-border/50 hover:bg-card/70 hover:border-secondary/30 hover:shadow-lg hover:shadow-secondary/5"
    >
      {/* Step number watermark */}
      <span className="absolute top-4 right-5 font-display text-4xl md:text-5xl font-bold select-none leading-none transition-colors duration-300 text-secondary/[0.07] group-hover:text-secondary/15">
        {step.step}
      </span>
